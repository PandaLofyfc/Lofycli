...
func main() {
	...
	if *versionFlag {
		fmt.Println("Lofycli Cli", buildinfo.InstallerTag, "("+buildinfo.InstallerGitHash+")")
	...
	if !SliceContainsFunc(switches, func(b *bool) bool { return *b }) {
		...
		go func() {
			...
			if IsSelfOutdated {
				...
				Log.Warn("To update, select the 'Update Lofycli' option to update, or run with --update-self")
			}
		}()
